...
#(
  ...
  output axdomain_t   [NoSnoopPorts-1:0] snoop_domain_o,
  ...
  `ACE_TYPEDEF_B_CHAN_T (int_ace_b_chan_t, int_id_t, user_t)
  ...
  for (genvar i = 0; i < NoGroups; i++) begin : gen_snoop
    ...
    int_ace_resps_t [1:0] ace_memory_resps;
    ...
    ace_ccu_snoop_path #(
      ...
      .snoop_domains_o      (snoop_domain_o [(2*i)+:2])
    );
    ...
    for (genvar j = 1; j < NoMemPortsPerGroup; j++) begin : gen_ace_to_axi
      ...
      `AXI_TO_ACE_ASSIGN_RESP(ace_memory_resps[j], axi_memory_resp [NoMemPortsPerGroup*i+j])
    end
  end
  ...
  for (genvar i = 0; i < NoGroups; i++) begin : gen_nosnoop
    ...
    `AXI_TO_ACE_ASSIGN_RESP(ace_nonsnooping_muxed_resp, axi_memory_resp[NoMemPortsPerGroup*i])
  end
  ...
  axi_mux #(
    ...
    .slv_aw_chan_t (int_axiaw_chan_t),
    .mst_aw_chan_t (mst_aw_chan_t),
    .w_chan_t      (w_chan_t),
    .slv_b_chan_t  (int_axib_chan_t),
    .mst_b_chan_t  (mst_b_chan_t),
    .slv_ar_chan_t (int_axiar_chan_t),
    .mst_ar_chan_t (mst_ar_chan_t),
    .slv_r_chan_t  (int_axir_chan_t),
    .mst_r_chan_t  (mst_r_chan_t),
    .slv_req_t     (int_axireq_t),
    .slv_resp_t    (int_axiresp_t),
    .mst_req_t     (mst_req_t),
